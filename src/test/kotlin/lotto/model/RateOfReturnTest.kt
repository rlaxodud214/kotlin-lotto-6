...
class RateOfReturnTest {
    ...
    fun `당첨금이 0원인 경우) 수익률 0,0 나오는지 확인`() {
        lottosMatchCount.update(LottoMatchCount(2, 0))
        val rateOfReturn = RateOfReturn(lottosMatchCount.result).calculate()
    ...
    fun `7장 구매해서 5천원 당첨된 경우) 소수점 2째자리에서 버리는 Case (71,43 to 71,4)`() {
        repeat(6) {
            lottosMatchCount.update(LottoMatchCount(2, 0))
        }.also { lottosMatchCount.update(LottoMatchCount(3, 0)) }
    ...
    fun `10장 구매해서 1등에 당첨된 경우) 단위 구분을 위한 ','가 출력되는지 확인`() {
        repeat(9) {
            lottosMatchCount.update(LottoMatchCount(2, 0))
        }.also { lottosMatchCount.update(LottoMatchCount(6, 0)) }
